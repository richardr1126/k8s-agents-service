...
# Configure the postgres MCP agent (Cosmere feed database analyst)
# Note: Don't pre-configure here, let the supervisor pass through the config
postgres_agent = mcp_agent

# Create supervisor workflow
workflow = create_supervisor(
    [resume_agent, web_research_agent, postgres_agent],
    model=model,
    prompt=(
        ...
        "When a user query is broad or ambiguous, decompose it into specific sub-questions and assign them to the appropriate agents one at a time. "
